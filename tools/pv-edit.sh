...
# for edit_pv in $(kubectl get pv -o jsonpath='{.items[*].metadata.name}'); do
# 	echo "Editing PV: ${edit_pv}"
# 	./pv-edit.sh $edit_pv
# done
...
PV_ORIG_FILE="./${PV}-orig.yaml"
PV_TMP_FILE="./${PV}-tmp.yaml"
...
# Use yq to edit spec.csi.volumeAttributes.portal
yq -i eval '.spec.csi.volumeAttributes.portal = "10.250.11.9:3260"' "${PV_TMP_FILE}"
